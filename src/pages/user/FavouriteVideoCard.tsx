...
import FavouriteVideoMenu from "./FavouriteVideoMenu";

type FavouriteVideoCardProps = {
  video: Video;
};

const FavouriteVideoCard: React.FC<FavouriteVideoCardProps> = ({ video }) => {
  const { addToWatchLater, removeFromFavorite, isInWatchLater } = useUserPageStore();
    ...
    <div className="flex gap-4 p-2 rounded-lg hover:bg-gray-700/50 transition-colors duration-200">
      ...
      <div className="shrink-0 ml-20">
        <FavouriteVideoMenu
          ...
          onAddToWatchLater={() => addToWatchLater(String(video.id))}
          onRemoveFromFavorite={() => removeFromFavorite(String(video.id))}
        />
      </div>
    </div>
  );
